...
class CUBE:
    def __init__(self, pos, scale, color):
        ...
        self.triangles = np.array(
            [
                ...
                [self.points[4], self.points[3], self.points[7]],
            ],
            int,
        )
    ...
    def rotate_point(self, point, angles):
        return np.dot(
            self.rotate_z(angles[2]),
            np.dot(self.rotate_y(angles[1]), np.dot(self.rotate_x(angles[0]), point)),
        )

    def scale_point(self, point):
        return np.array(
            [
                point[0] * self.scale + self.cx,
                point[1] * self.scale + self.cy,
                point[2],
            ],
            np.float16,
        )
    ...
    def shade(self, shade_factor):
        s = [
            self.color[0] * (1 - shade_factor),
            self.color[1] * (1 - shade_factor),
            self.color[2] * (1 - shade_factor),
        ]
        if s[0] > 255:
            s[0] = 255
        if s[1] > 255:
...
def main():
    ...
    while running:
        ...
        c1.render(screen, [30, -a, np.sin(-a * 0.053) * 40])
